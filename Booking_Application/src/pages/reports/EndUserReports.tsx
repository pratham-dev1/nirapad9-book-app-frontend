...
import {  GET_END_USER_REPORTS, GET_OPEN_AVAILABILITY_TAG,} from "../../constants/Urls";
...
const EndUserReports: React.FC<{ value: number, index: number }> = ({ value, index }) => {
  ...
  const [params, setParams] = useState({
    ...
    sortingOrder: "desc",
    tagId:undefined //Need to pass proper value here
  });
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery(["end-user-reports", params], () =>
    request(GET_END_USER_REPORTS, "get", params),
    {
      enabled: (value === index)
    }
  );
  ...
  useEffect(() => {
    if (endUsersData?.data) {
      const DATA = endUsersData?.data?.map((item: any) => ({
        ...item,
  ...
  const handleApplyFilter = () => {
      ...
      setParams((prev) => ({ ...prev, startDateFilter: startDateTime, endDateFilter: endDateTime,tagId:selectedTag?.id,...getBookedFilterStatus() }))
  };
