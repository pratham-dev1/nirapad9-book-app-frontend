...
import { DELETE_BOOKED_OPEN_AVAILABILITY, DELETE_EVENT_HUB_EVENT, DELETE_OPEN_AVAILABILITY, GET_EVENT_HUB_HISTORY, GET_TALENT_PARTNER_HISTORY, GET_TIMEZONES, UPDATE_BOOKED_OPEN_SLOT, UPDATE_OPEN_SLOT } from "../constants/Urls";
...
const EventHubHistory: React.FC<{ value: number, index: number }> = ({ value, index })=> {
  ...
  const [historyView, setHistoryView] = useState({id:3, name:'Event Hub History'})
...
//   })
  ...
  const {
    ...
    isRefetching,
    refetch,
  } = useQuery(["event-hub-history", params], () =>
    request(GET_EVENT_HUB_HISTORY, "get", params),
    {
      enabled: (value === index)
    }
  );
  ...
  const handleInputChange = (value: any) => {
    setParams(prev => (
      { ...prev, 
        ...(value?.id === 1 ? { bookedFilter: true } : 
          value?.id === 2 ? { bookedFilter: false } : 
          value?.id === 3 ? { bookedFilter: null } : 
          value?.id === 4 ? { bookedFilter: 'cancelled' } :
          value?.id === 5 ? {bookedFilter: 'cancelRequested'} : {} 
  )}));
  ...
  return (
    ...
    {isDeleteLoading && <Loader />}
      {historyView?.name === "Event Hub History" ? 
      <>
      ...
      </>: <>{state.userType === 4 ? <OpenAvailabilityHistory value={value} index={index} /> : state.userType === 1 ? (historyView.id == 1 ? <TpHistory value={value} index={index}/>: <OpenAvailabilityHistory value={value} index={index}/> ): state.userType === 2 ? (historyView.id == 1 ? <RecruiterHistory value={value} index={index}/>: <OpenAvailabilityHistory value={value} index={index}/>) :null} </>  }
...
export default EventHubHistory;
