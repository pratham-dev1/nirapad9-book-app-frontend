...
import { GET_USER_LIST, DELETE_USERS, RESTORE_USER, CREATE_BULK_USERS, RESEND_VERIFICATION_LINK, GET_ALL_USER_LIST_FOR_GRID, ENABLE_OR_DISABLE_CREDENTIALS } from "../../../constants/Urls";
...
const UserList = () => {
  ...
  const [openCredentialDisbledDialog, setOpenCredentialDisbledDialog] = useState(false);
  const [credentialsSettingData, setCredentialsSettingData] = useState<any>()
  ...
  const {data} = useQuery(['user-list',params],()=>request(GET_ALL_USER_LIST_FOR_GRID,"get",params))
...
const { mutate: mutateDisabledOrEnabledCredentials, isLoading: isCredentialSettingLoading } = useMutation((body: object) => request(ENABLE_OR_DISABLE_CREDENTIALS, "put", body),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('user-list');
      },
...
const handlePagination = (pageInfo: GridPaginationModel) => {
  ...
  const columns: GridColDef[] = [
    ...
    {
        field: "isCredentialDisabled",
        headerName: "Credentials Disabled/Enabled",
        width: 200,
        sortable: false,
        renderCell: (params) => {
          const {isCredentialsDisabled , id} = params.row
          const handleClick = () => {
            setOpenCredentialDisbledDialog(true)
            setCredentialsSettingData({userId: id, value: !isCredentialsDisabled})
          }
          return <Button variant="text" onClick={handleClick} disabled={params.row.isDeleted} style={{backgroundColor: isCredentialsDisabled ? 'red' : 'green'}}>{isCredentialsDisabled ? 'Disabled' : 'Enabled'}</Button>
        }
      },
  ];
 ...
 const handleSortModelChange =(sortInfo: GridSortModel) => {
  ...
  const handleDisabledOrEnabledCredentials = () => {
    setOpenCredentialDisbledDialog(false)
    mutateDisabledOrEnabledCredentials(credentialsSettingData)
  }
 ...
 const CustomToolbar = () => { 
  ...
  return (
    <>
      ...
      <Dialog
        open={openCredentialDisbledDialog}
        aria-labelledby="alert-dialog-title"
        aria-describedby="alert-dialog-description"
      >
        <div className="popup-header">
          <h2><WarningIcon /> <span className="pl-10">Please Confirm</span></h2>
          <CloseIcon onClick={() => setOpenCredentialDisbledDialog(false)} />
        </div>
        <DialogContent>
          <h3 className="text-center">Are you sure you want to update login setting for the selected user?</h3>
        </DialogContent>
        <DialogActions>
          <CustomButton onClick={handleDisabledOrEnabledCredentials} disabled={isCredentialSettingLoading} label="Confirm" className="primary_btns" />
          <CustomButton onClick={() => setOpenCredentialDisbledDialog(false)}  label="cancel" className="secondary_btns" />
        </DialogActions>
      </Dialog>
    </>
  );
